...
$icono_categoria = isset($_POST['icono'])?$_POST['icono']:false;
...
if(isset($_POST['registro']) && $_POST['registro']=='nuevo'){
	try {
		...
		$stmt->bind_param("ss",$nombre_categoria,$icono_categoria);
		$stmt->execute();
...
if(isset($_POST['registro']) && $_POST['registro']=='actualizar'){
	...
	try {
		...
		$stmt->bind_param('ssi',$nombre_categoria,$icono_categoria,$id_registro);
		$stmt->execute();
		if($stmt->affected_rows){
			$respuesta = array(
				'respuesta'=>'exito',
